...
export default function ArrowButton({ direction, label, emphasized = false, onClick, ariaLabel }: Props): JSX.Element {
  if (direction === 'up') {
    return (
      <button onClick={onClick} aria-label={ariaLabel ?? 'Up'} className="fixed top-24 md:top-16 left-1/2 -translate-x-1/2 z-40">
  ...
  return (
    <button onClick={onClick} aria-label={ariaLabel ?? 'Down'} className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40">
